...
export function CreateChurchForm({ users, onComplete }: { users: { id: string; email: string | null }[]; onComplete: () => void }) {
  ...
  return <form onSubmit={submit} noValidate className="grid gap-3 rounded-xl bg-white p-4 shadow-sm ring-1 ring-gray-200 md:grid-cols-3">
    ...
    <p className="text-xs text-gray-500 md:col-span-2">Zona horaria: America/Mexico_City</p><button disabled={submitting} className="min-h-11 rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">Crear iglesia</button>
  </form>
}
