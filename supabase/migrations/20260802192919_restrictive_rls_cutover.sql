...
REVOKE EXECUTE ON FUNCTION public.is_user_active(uuid), public.is_platform_admin(), public.activate_current_user(), public.is_church_member(uuid), public.has_church_role(uuid, public.church_role), public.is_curator(), public.create_church(text, text, uuid), public.auto_adopt_canonical_song() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.is_user_active(uuid), public.is_platform_admin(), public.is_church_member(uuid), public.has_church_role(uuid, public.church_role), public.is_curator(), public.create_church(text, text, uuid), public.auto_adopt_canonical_song() TO authenticated, service_role;
